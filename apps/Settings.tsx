...
const Settings: React.FC = () => {
  ...
  const [localBrainMode, setLocalBrainMode] = useState<'off' | 'follow' | 'independent'>(apiConfig.characterBrainApi?.mode || 'follow');
  const [localBrainUrl, setLocalBrainUrl] = useState(apiConfig.characterBrainApi?.baseUrl || '');
  const [localBrainKey, setLocalBrainKey] = useState(apiConfig.characterBrainApi?.apiKey || '');
  const [localBrainModel, setLocalBrainModel] = useState(apiConfig.characterBrainApi?.model || '');
  const [localBrainLiveliness, setLocalBrainLiveliness] = useState(apiConfig.characterBrainApi?.liveliness ?? 70);
  const [brainStatusMsg, setBrainStatusMsg] = useState('');
  ...
  useEffect(() => {
      setLocalBrainMode(apiConfig.characterBrainApi?.mode || 'follow');
      setLocalBrainUrl(apiConfig.characterBrainApi?.baseUrl || '');
      setLocalBrainKey(apiConfig.characterBrainApi?.apiKey || '');
      setLocalBrainModel(apiConfig.characterBrainApi?.model || '');
      setLocalBrainLiveliness(apiConfig.characterBrainApi?.liveliness ?? 70);
  }, [apiConfig.characterBrainApi?.mode, apiConfig.characterBrainApi?.baseUrl, apiConfig.characterBrainApi?.apiKey, apiConfig.characterBrainApi?.model, apiConfig.characterBrainApi?.liveliness]);
  ...
  return (
    <div className="h-full w-full bg-[#f3f4f8] flex flex-col font-light relative isolate">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 no-scrollbar pb-20">
        ...
        <SettingsSection
            title="独立翻译 API"
            icon={<div className="p-2 bg-sky-100/70 rounded-xl text-sky-600">🌐</div>}
        >
            <div className="space-y-3">
                <p className="text-[11px] text-slate-400 leading-relaxed">聊天里的“翻译”按钮可单独调用这里的模型，不占用主聊天 API。支持 OpenAI 兼容的 /chat/completions 接口。</p>
                <input value={apiConfig.translationApi?.baseUrl || ''} onChange={e=>updateApiConfig({...apiConfig, translationApi:{enabled:true,baseUrl:e.target.value,apiKey:apiConfig.translationApi?.apiKey||'',model:apiConfig.translationApi?.model||'',timeoutMs:apiConfig.translationApi?.timeoutMs||30000}})} placeholder="API URL（OpenAI 兼容）" className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm" />
                <input type="password" value={apiConfig.translationApi?.apiKey || ''} onChange={e=>updateApiConfig({...apiConfig, translationApi:{enabled:true,baseUrl:apiConfig.translationApi?.baseUrl||'',apiKey:e.target.value,model:apiConfig.translationApi?.model||'',timeoutMs:apiConfig.translationApi?.timeoutMs||30000}})} placeholder="API Key" className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm font-mono" />
                <input value={apiConfig.translationApi?.model || ''} onChange={e=>updateApiConfig({...apiConfig, translationApi:{enabled:true,baseUrl:apiConfig.translationApi?.baseUrl||'',apiKey:apiConfig.translationApi?.apiKey||'',model:e.target.value,timeoutMs:apiConfig.translationApi?.timeoutMs||30000}})} placeholder="模型名（例如便宜的小模型）" className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm" />
                <button type="button" onClick={()=>updateApiConfig({...apiConfig,translationApi:{enabled:true,baseUrl:apiConfig.translationApi?.baseUrl||'',apiKey:apiConfig.translationApi?.apiKey||'',model:apiConfig.translationApi?.model||'',timeoutMs:apiConfig.translationApi?.timeoutMs||30000}})} className="w-full py-2.5 rounded-xl bg-sky-500 text-white text-xs font-bold">启用独立翻译 API</button>
            </div>
        </SettingsSection>

        <SettingsSection
            title="角色大脑 / 活人感"
            badge={<span className={`text-[9px] font-bold px-2 py-1 rounded-full ${localBrainMode === 'off' ? 'bg-slate-100 text-slate-400' : 'bg-emerald-100 text-emerald-600'}`}>{localBrainMode === 'off' ? '本地兜底' : localBrainMode === 'follow' ? '跟随主 API' : '独立 API'}</span>}
            icon={<div className="p-2 bg-emerald-100/60 rounded-xl text-emerald-600">🧠</div>}
        >
            <div className="space-y-3">
                <p className="text-[11px] text-slate-400 leading-relaxed">控制角色的生活连续性、主动话题、聊天节奏与状态判断。关闭或独立 API 失败时仍保留本地活人感规则，不影响正常聊天。</p>
                <div className="grid grid-cols-3 gap-2">
                    {(['off','follow','independent'] as const).map(mode => <button key={mode} type="button" onClick={() => setLocalBrainMode(mode)} className={`py-2 rounded-xl text-xs font-semibold ${localBrainMode === mode ? 'bg-emerald-500 text-white' : 'bg-white/60 border border-slate-200 text-slate-500'}`}>{mode === 'off' ? '仅本地' : mode === 'follow' ? '跟随聊天' : '独立 API'}</button>)}
                </div>
                {localBrainMode === 'independent' && <>
                    <input value={localBrainUrl} onChange={e=>setLocalBrainUrl(e.target.value)} placeholder="API URL（OpenAI 兼容）" className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm" />
                    <input type="password" value={localBrainKey} onChange={e=>setLocalBrainKey(e.target.value)} placeholder="API Key" className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm font-mono" />
                    <input value={localBrainModel} onChange={e=>setLocalBrainModel(e.target.value)} placeholder="模型名（建议便宜的小模型）" className="w-full bg-white/50 border border-slate-200/60 rounded-xl px-4 py-2.5 text-sm" />
                </>}
                <label className="block text-[11px] text-slate-500">活人感强度：{localBrainLiveliness}</label>
                <input type="range" min="0" max="100" value={localBrainLiveliness} onChange={e=>setLocalBrainLiveliness(Number(e.target.value))} className="w-full" />
                <button type="button" onClick={() => { updateApiConfig({ ...apiConfig, characterBrainApi: { mode: localBrainMode, baseUrl: localBrainUrl.trim(), apiKey: localBrainKey.trim(), model: localBrainModel.trim(), liveliness: localBrainLiveliness } }); setBrainStatusMsg('已保存'); setTimeout(()=>setBrainStatusMsg(''),1500); }} className="w-full py-2.5 rounded-xl bg-emerald-500 text-white text-xs font-bold">{brainStatusMsg || '保存角色大脑设置'}</button>
            </div>
        </SettingsSection>
