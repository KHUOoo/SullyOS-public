...
import ChatAssetStyles from '../components/chat/ChatAssetStyles';
import ChatControlStyles from '../components/chat/ChatControlStyles';
...
import { hasPendingAssistantImageIntent } from '../utils/assistantImageIntent';
...
type AssistantPhotoActivityPhase = 'idle' | 'generating_photo' | 'sending_photo' | 'failed';
...
const Chat: React.FC = () => {
    ...
    const [assistantPhotoActivity, setAssistantPhotoActivity] = useState<{ charId: string; phase: Exclude<AssistantPhotoActivityPhase, 'idle'> } | null>(null);
    const assistantPhotoFailureTimerRef = useRef<number | null>(null);
    const updateAssistantPhotoActivity = useCallback((charId: string, phase: AssistantPhotoActivityPhase) => {
        if (assistantPhotoFailureTimerRef.current !== null) {
            window.clearTimeout(assistantPhotoFailureTimerRef.current);
            assistantPhotoFailureTimerRef.current = null;
        }
        if (phase === 'idle') {
            setAssistantPhotoActivity(current => current?.charId === charId ? null : current);
            return;
        }
        setAssistantPhotoActivity({ charId, phase });
        if (phase === 'failed') {
            assistantPhotoFailureTimerRef.current = window.setTimeout(() => {
                setAssistantPhotoActivity(current =>
                    current?.charId === charId && current.phase === 'failed' ? null : current
                );
                assistantPhotoFailureTimerRef.current = null;
            }, 2600);
        }
    ...
    // Once the user leaves the bottom, background reload/media layout must never
    // steal their reading position. It is re-armed only when they return near bottom.
    ...
    useEffect(() => {
        ...
        setAssistantPhotoActivity(current =>
            current && current.charId !== activeCharacterId ? null : current
        );
    }, [activeCharacterId]);
    useEffect(() => () => {
        if (assistantPhotoFailureTimerRef.current !== null) {
    ...
    const [translationLoadingIds, setTranslationLoadingIds] = useState<Set<number>>(new Set());
    ...
    const handleTranslateToggle = useCallback(async (msgId: number) => {
        const msg = messages.find(m => m.id === msgId);
        if (!msg) return;
        if (translatedContents[msgId]) {
        ...
        if (!apiConfig.translationApi?.enabled) {
            setShowingTargetIds(prev => {
                const next = new Set(prev);
                if (next.has(msgId)) next.delete(msgId);
                else next.add(msgId);
        ...
        if (translationLoadingIds.has(msgId)) return;
        const raw = String(msg.content || '').replace(/%%BILINGUAL%%[\\s\\S]*$/i, '').trim();
        if (!raw) return;
        setTranslationLoadingIds(prev => new Set(prev).add(msgId));
        try {
            const translated = await translateText(
                raw,
                translateSourceLang,
                translateTargetLang,
                apiConfig.translationApi,
            );
            setTranslatedContents(prev => ({ ...prev, [msgId]: translated }));
            setShowingTargetIds(prev => new Set(prev).add(msgId));
        } catch (e) {
            addToast(e instanceof Error ? e.message : '翻译失败', 'error');
        } finally {
            setTranslationLoadingIds(prev => {
                const next = new Set(prev);
                next.delete(msgId);
                return next;
            });
    ...
    const reloadMessages = useCallback(async (requestedVisibleCount: number) => {
        ...
        const applyResult = (recent: Message[], totalCount: number) => {
            ...
            const nextMessages = chatScopeMsgs.slice(-requestedVisibleCount);
            // Background refreshes are frequent (timestamps, instant-chat state, etc.).
            // Do not replace an identical message array: doing so needlessly re-commits
            // the whole chat list and can make custom message chrome visibly flash.
            setMessages(prev => {
                if (
        ...
        try {
            ...
            try {
                ...
                // Keep the last successfully rendered messages on screen. A transient IndexedDB
                // failure must not look like the conversation was deleted.
                console.error('[chat] failed to load messages after retry', {
                    charId: charIdAtStart,
                    error: retryError,
                    firstError: e,
                });
                addToast('聊天记录读取失败，已保留当前消息，请稍后重试', 'error');
            }
        }
    ...
    // This is character-entry initialization, not a generic reload effect.
    // reloadMessages indirectly depends on OSContext callbacks (e.g. addToast), whose
    // identity may change when unrelated provider state updates. Depending on it here
    // would re-run initialization, clear the whole message list and reset scroll.
    ...
    useLayoutEffect(() => {
        ...
        if (currentLastId !== lastMsgIdRef.current) {
            // The first loaded history establishes the baseline. It may position the
            // conversation at the bottom once, but subsequent reloads may only follow
            // genuinely new messages while the user is still reading at the bottom.
            const establishingBaseline = lastMsgIdRef.current === null;
            if (windowedFocusMsgId === null && (establishingBaseline || !userAwayFromBottomRef.current)) {
    ...
    const handleChatScroll = useCallback(() => {
        ...
        userAwayFromBottomRef.current = distanceFromBottom > 96;
        if (userAwayFromBottomRef.current) pendingMediaAutoScrollIdRef.current = null;
    ...
    const handleMessageMediaLoad = useCallback((messageId: number) => {
        if (userAwayFromBottomRef.current || windowedFocusMsgId !== null || pendingMediaAutoScrollIdRef.current !== messageId) return;
    ...
    const sendText = async (customContent?: string, customType?: MessageType, metadata?: any) => {
        ...
        const explicitPhotoRequest = type === 'text' && apiConfig.imageGenApi?.forcePhotoOnExplicitRequest !== false && isCharacterPhotoRequestText(text, char?.name);
    ...
    useEffect(() => {
        ...
        const assistantImageIntentMessage = assistantReplies.find(message => !!hasPendingAssistantImageIntent(message));
        const assistantImageIntent = assistantImageIntentMessage
            ? hasPendingAssistantImageIntent(assistantImageIntentMessage)
            : null;
        ...
        const explicitPhotoForceEnabled = apiConfig.imageGenApi?.forcePhotoOnExplicitRequest !== false;
        const forceImage = !!assistantImageIntent || (
            explicitPhotoForceEnabled
            && pendingTurnUsers.some(
                message => message.metadata?.contextualImage?.requested === true || isCharacterPhotoRequestText(message.content, char.name),
            )
        );
        contextualImageProcessingRef.current.add(anchor.id);
        if (forceImage) updateAssistantPhotoActivity(char.id, 'generating_photo');

        void (async () => {
            let imageAttempted = forceImage;
            ...
            try {
                ...
                if (assistantImageIntentMessage?.id) {
                    await DB.updateMessageMetadata(assistantImageIntentMessage.id, previous => ({
                        ...(previous || {}),
                        assistantImageIntent: {
                            ...(previous?.assistantImageIntent || {}),
                            ...(assistantImageIntent || {}),
                            status: 'processing',
                            updatedAt: Date.now(),
                        },
                ...
                let plan;
                if (assistantImageIntent) {
                    // 角色已经明确“要发这张图”并给出了画面描述：不要再让规划模型投票，
                    // 直接把这段内部描述交给现有深图管线。
                    plan = {
                        sendImage: true,
                        mode: 'free' as CharacterPhotoMode,
                        scene: assistantImageIntent.scene,
                    };
                } else if (forceImage) {
                    // 用户已经明确要看角色 / 要照片：这是强制动作，不再交给规划模型二次投票。
                    // 最终摄影提示词仍由现有 contextual prompt builder 读取当前时间、场景、聊天和角色设定。
                    const explicitRequestText = pendingTurnUsers
                        .filter(message => message.metadata?.contextualImage?.requested === true || isCharacterPhotoRequestText(message.content, char.name))
                    ...
                    plan = {
                        ...
                        mode: 'free' as CharacterPhotoMode,
                        scene: [
                            `用户明确想看 ${char.name}，请让 ${char.name} 现在直接拍一张并发给用户。`,
                            '严格延续当前聊天发生的时间、地点、人物状态、正在做的事、服装与情绪；如果上下文没有说明，就采用角色此刻最自然可信的生活场景。',
                            explicitRequestText ? `用户原话：${explicitRequestText}` : '',
                        ].filter(Boolean).join(' '),
                    };
                } else if (apiConfig.imageGenApi?.allowProactiveChatPhotos !== false) {
                    ...
                    plan = { sendImage: false, mode: 'free' as CharacterPhotoMode, scene: '' };
                }
                if (!plan.sendImage) {
                    if (forceImage) updateAssistantPhotoActivity(char.id, 'idle');
                    await setTurnStatus('done', 'text_only');
                    return;
                }
                imageAttempted = true;
                updateAssistantPhotoActivity(char.id, 'generating_photo');
                ...
                updateAssistantPhotoActivity(char.id, 'sending_photo');
                let stored = image.startsWith('data:') ? await migrateDataUrlToRef(image) : image;
                if (/^https?:\/\//i.test(image)) {
                    try {
                        const response = await fetch(image);
                ...
                const sourceMessageId = await DB.saveMessage({
                    ...
                    content: stored,
                    metadata: {
                        source: assistantImageIntent ? 'assistant_intent_character_photo' : 'contextual_character_photo',
                        sourceUserMessageId: anchor.id,
                        sourceAssistantMessageId: assistantImageIntentMessage?.id,
                        assistantImageIntentSource: assistantImageIntent?.source,
                        photoMode: plan.mode,
                        prompt,
                        imageGenSnapshot: apiConfig.imageGenApi ? {
                            model: apiConfig.imageGenApi.model,
                ...
                if (assistantImageIntentMessage?.id) {
                    await DB.updateMessageMetadata(assistantImageIntentMessage.id, previous => ({
                        ...
                        assistantImageIntent: {
                            ...
                            status: 'done',
                            outcome: 'image_sent',
                            imageMessageId: sourceMessageId,
                            updatedAt: Date.now(),
                        },
                    }));
                }
                ...
                updateAssistantPhotoActivity(char.id, 'idle');
            } catch (error) {
                console.warn('[Chat] contextual image generation failed; kept short text reply', error);
                if (imageAttempted) updateAssistantPhotoActivity(char.id, 'failed');
                ...
                if (forceImage) addToast('照片生成失败，图片描述已隐藏；可以稍后重试', 'error');
            } finally {
                contextualImageProcessingRef.current.delete(anchor.id);
            }
        })();
    }, [apiConfig, char, groups, instantChatPending, isTyping, localDateKey, messages, realtimeConfig, reloadMessages, userProfile, addToast, updateAssistantPhotoActivity]);
    ...
    return (
        ...
        >
             ...
             <ChatAssetStyles globalAssets={baseOsTheme.chatAssetDecorations} characterAssets={char.chatAssetDecorations}/>
             <ChatControlStyles globalLayout={baseOsTheme.chatControlLayout} characterLayout={char.chatControlLayout}/>
             ...
             {(osTheme.chatChromeCustomCss || char.chromeCustomCss || activeTheme.customCss || (char as any).thinkingChainCustomCss) && (
               <style>{`
                 ...
                 /* 消息区只提供稳定 selector。不要常驻强制 display/opacity：
                    用户主题可能合法地用透明度/动画美化消息，永久 !important 会与其反复争夺样式。 */
               `}</style>
             )}
             ...
             <ChatHeader
                ...
                isTyping={isTyping || instantChatPending || isProactiveComposing}
                photoActivityState={assistantPhotoActivity?.charId === char.id ? assistantPhotoActivity.phase : 'idle'}
            ...
            <div ref={scrollRef} onScroll={handleChatScroll} onClick={() => { if (inputPreferences.autoReply) setShowPanel('none'); }} className="sully-chat-messages flex-1 overflow-y-auto overflow-x-hidden pt-6 pb-6 no-scrollbar" style={{ backgroundImage: activeTheme.type === 'custom' && activeTheme.user.backgroundImage ? 'none' : undefined }}>
                ...
                {renderedMessages.map((m, i) => {
                    ...
                    return (
                        ...
                        <MessageItem
                            ...
                            translatedContent={translatedContents[m.id]}
                            translationLoading={translationLoadingIds.has(m.id)}
                            translationExpanded={translationExpanded}
                            ...
                            // Persisted history must render immediately. The row wrapper above owns the
                            // one-shot animation for genuinely new messages; letting MessageItem animate too
                            // makes every existing bubble flash again when entering/reloading the chat.
                ...
                {(isTyping || instantChatPending || recallStatus || searchStatus || diaryStatus || isProactiveComposing || (assistantPhotoActivity?.charId === char.id && (assistantPhotoActivity.phase === 'generating_photo' || assistantPhotoActivity.phase === 'sending_photo'))) && !selectionMode && (
