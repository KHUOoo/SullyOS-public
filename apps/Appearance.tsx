                    ...
                    <div className="mt-4 border-t border-slate-100 pt-4 space-y-4">
                        <div>
                            <div className="flex items-center justify-between gap-3 mb-2">
                                <div>
                                    <div className="text-xs font-bold text-slate-600">上下位置</div>
                                    <div className="text-[9px] text-slate-400 mt-0.5">只移动时间 / Wi‑Fi / 电量，不移动状态栏背景。</div>
                                </div>
                                <div className="text-[10px] font-mono text-slate-500">{Number(theme.statusBarOffsetY || 0) > 0 ? '+' : ''}{Number(theme.statusBarOffsetY || 0)}px</div>
                            </div>
                            <input
                                type="range"
                                min="-24"
                                max="40"
                                step="1"
                                value={Number(theme.statusBarOffsetY || 0)}
                                onChange={e => updateTheme({ statusBarOffsetY: Number(e.target.value) })}
                                className="w-full accent-primary"
                                aria-label="状态栏上下位置"
                            />
                            <div className="mt-1 flex justify-between text-[9px] text-slate-300"><span>更靠上</span><span>默认</span><span>更靠下</span></div>
                        </div>

                        <div>
                            <div className="flex items-center justify-between gap-3 mb-2">
                                <div>
                                    <div className="text-xs font-bold text-slate-600">时间 / Wi‑Fi / 电量颜色</div>
                                    <div className="text-[9px] text-slate-400 mt-0.5">不设置时继续跟随当前主题文字颜色。</div>
                                </div>
                                <button
                                    type="button"
                                    onClick={() => updateTheme({ statusBarColor: undefined })}
                            ...
                            <div className="flex items-center gap-3">
                                <input
                                    type="color"
                                    value={/^#[0-9a-f]{6}$/i.test(theme.statusBarColor || '') ? theme.statusBarColor! : '#ffffff'}
                                ...
                                <input
                                    type="text"
                                    value={theme.statusBarColor || ''}
                                    onChange={e => updateTheme({ statusBarColor: e.target.value.trim() || undefined })}
                                    placeholder={theme.contentColor || '#ffffff'}
                                    className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 outline-none focus:border-primary/40 focus:bg-white"
                                    aria-label="状态栏颜色值"
                                />
                            </div>
                            <p className="mt-2 text-[9px] leading-relaxed text-slate-400">可以直接用取色器，也可以输入 #HEX、rgb(...)、hsl(...) 等 CSS 颜色。</p>
                        </div>

                        {(theme.statusBarOffsetY || theme.statusBarColor) && (
                            <button
                                type="button"
                                onClick={() => updateTheme({ statusBarOffsetY: undefined, statusBarColor: undefined })}
                                className="w-full rounded-xl bg-slate-50 py-2 text-[10px] font-bold text-slate-500 active:scale-[0.99]"
                            >
                                恢复状态栏位置与颜色默认值
                            </button>
                        )}
