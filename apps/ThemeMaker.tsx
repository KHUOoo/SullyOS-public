...
import { bubbleFrameContainerStyle, bubbleFrameLayerStyle, hasBubbleFrame, resolveBubbleFrameValues } from '../utils/bubbleNineSlice';
...
const readImageSize = (src: string): Promise<{ width: number; height: number } | null> => new Promise(resolve => {
    ...
    image.onload = () => resolve({ width: Math.max(1, image.naturalWidth || image.width), height: Math.max(1, image.naturalHeight || image.height) });
...
async function bubbleStyleToBlobRefs(style: BubbleStyle): Promise<BubbleStyle> {
    ...
    next.frameImage = await toRef(next.frameImage);
...
const BubbleFrameLayer: React.FC<{ style: BubbleStyle }> = ({ style }) => {
    const url = useBlobRefUrl(style.frameImage);
    if (!url || !style.frameImage) return null;
    return <div className="sully-bubble-nine-slice" style={bubbleFrameLayerStyle(style, url)} />;
};

type BubbleGuideMode = 'stretch' | 'content';
type BubbleGuideEdge = 'left' | 'right' | 'top' | 'bottom';

const BubbleNineSliceGuide: React.FC<{
    style: BubbleStyle;
    onPatch: (patch: Partial<BubbleStyle>) => void;
}> = ({ style, onPatch }) => {
    const url = useBlobRefUrl(style.frameImage);
    const values = resolveBubbleFrameValues(style);
    const [mode, setMode] = useState<BubbleGuideMode>('stretch');
    const [naturalSize, setNaturalSize] = useState({ width: 1, height: 1 });
    const boxRef = useRef<HTMLDivElement>(null);
    const draggingRef = useRef<BubbleGuideEdge | null>(null);

    if (!url) return null;

    const clamp = (value: number, min: number, max: number) => Math.max(min, Math.min(max, value));
    const sourceWidth = Math.max(1, naturalSize.width);
    const sourceHeight = Math.max(1, naturalSize.height);
    const visualSliceLeft = style.frameFlipX ? values.sliceRight : values.sliceLeft;
    const visualSliceRight = style.frameFlipX ? values.sliceLeft : values.sliceRight;
    const visualContentLeftRaw = style.frameFlipX ? values.contentRight : values.contentLeft;
    const visualContentRightRaw = style.frameFlipX ? values.contentLeft : values.contentRight;
    const defaultContentLeft = Math.round(visualSliceLeft * 0.72);
    const defaultContentRight = Math.round(visualSliceRight * 0.72);
    const defaultContentTop = Math.round(values.sliceTop * 0.68);
    const defaultContentBottom = Math.round(values.sliceBottom * 0.68);
    const visualContentLeft = clamp(visualContentLeftRaw ?? defaultContentLeft, 0, visualSliceLeft);
    const visualContentRight = clamp(visualContentRightRaw ?? defaultContentRight, 0, visualSliceRight);
    const contentTop = clamp(values.contentTop ?? defaultContentTop, 0, values.sliceTop);
    const contentBottom = clamp(values.contentBottom ?? defaultContentBottom, 0, values.sliceBottom);

    const stretchPositions = {
        left: clamp((visualSliceLeft / sourceWidth) * 100, 0, 49.5),
        right: clamp(100 - (visualSliceRight / sourceWidth) * 100, 50.5, 100),
        top: clamp((values.sliceTop / sourceHeight) * 100, 0, 49.5),
        bottom: clamp(100 - (values.sliceBottom / sourceHeight) * 100, 50.5, 100),
    };
    const contentPositions = {
        left: clamp((visualContentLeft / sourceWidth) * 100, 0, stretchPositions.left),
        right: clamp(100 - (visualContentRight / sourceWidth) * 100, stretchPositions.right, 100),
        top: clamp((contentTop / sourceHeight) * 100, 0, stretchPositions.top),
        bottom: clamp(100 - (contentBottom / sourceHeight) * 100, stretchPositions.bottom, 100),
    };
    const positions = mode === 'stretch' ? stretchPositions : contentPositions;

    const initializeContentRegion = () => {
        if (
            values.contentLeft !== undefined ||
            values.contentRight !== undefined ||
            values.contentTop !== undefined ||
            values.contentBottom !== undefined
        ) return;
        onPatch({
            frameContentLeft: style.frameFlipX ? defaultContentRight : defaultContentLeft,
            frameContentRight: style.frameFlipX ? defaultContentLeft : defaultContentRight,
            frameContentTop: defaultContentTop,
            frameContentBottom: defaultContentBottom,
        });
    };

    const applyPointer = (edge: BubbleGuideEdge, clientX: number, clientY: number) => {
        const rect = boxRef.current?.getBoundingClientRect();
        if (!rect || rect.width <= 0 || rect.height <= 0) return;
        const x = clamp(clientX - rect.left, 0, rect.width);
        const y = clamp(clientY - rect.top, 0, rect.height);
        const sourceX = (x / rect.width) * sourceWidth;
        const sourceY = (y / rect.height) * sourceHeight;

        if (mode === 'stretch') {
            if (edge === 'left') {
                const max = Math.max(0, sourceWidth - visualSliceRight - 2);
                const next = Math.round(clamp(sourceX, 0, max));
                onPatch(style.frameFlipX ? { frameSliceRight: next } : { frameSliceLeft: next });
            } else if (edge === 'right') {
                const candidate = sourceWidth - sourceX;
                const max = Math.max(0, sourceWidth - visualSliceLeft - 2);
                const next = Math.round(clamp(candidate, 0, max));
                onPatch(style.frameFlipX ? { frameSliceLeft: next } : { frameSliceRight: next });
            } else if (edge === 'top') {
                const max = Math.max(0, sourceHeight - values.sliceBottom - 2);
                onPatch({ frameSliceTop: Math.round(clamp(sourceY, 0, max)) });
            } else {
                const candidate = sourceHeight - sourceY;
                const max = Math.max(0, sourceHeight - values.sliceTop - 2);
                onPatch({ frameSliceBottom: Math.round(clamp(candidate, 0, max)) });
            }
            return;
        }

        if (edge === 'left') {
            const next = Math.round(clamp(sourceX, 0, visualSliceLeft));
            onPatch(style.frameFlipX ? { frameContentRight: next } : { frameContentLeft: next });
        } else if (edge === 'right') {
            const next = Math.round(clamp(sourceWidth - sourceX, 0, visualSliceRight));
            onPatch(style.frameFlipX ? { frameContentLeft: next } : { frameContentRight: next });
        } else if (edge === 'top') {
            onPatch({ frameContentTop: Math.round(clamp(sourceY, 0, values.sliceTop)) });
        } else {
            onPatch({ frameContentBottom: Math.round(clamp(sourceHeight - sourceY, 0, values.sliceBottom)) });
        }
    };

    const line = (edge: BubbleGuideEdge, position: number) => {
        const vertical = edge === 'left' || edge === 'right';
        const color = mode === 'stretch' ? '#f43f5e' : '#38bdf8';
        const stylePos: React.CSSProperties = vertical
            ? { left: `${position}%`, top: 0, bottom: 0, width: 2, backgroundColor: color }
            : { top: `${position}%`, left: 0, right: 0, height: 2, backgroundColor: color };
        return (
            <div key={edge} className="absolute z-20 shadow-[0_0_0_1px_rgba(255,255,255,.75)]" style={stylePos}>
                <button
                    type="button"
                    aria-label={`拖动${edge}参考线`}
                    onPointerDown={(event) => {
                    ...
                    onPointerUp={(event) => {
                        if (draggingRef.current === edge) applyPointer(edge, event.clientX, event.clientY);
                        draggingRef.current = null;
                    }}
                    onPointerCancel={() => { draggingRef.current = null; }}
                    className="absolute left-1/2 top-1/2 z-30 h-7 w-7 -translate-x-1/2 -translate-y-1/2 rounded-full border-[3px] bg-white shadow-md active:scale-110"
                    style={{ touchAction: 'none', borderColor: color }}
                />
            </div>
        );
    };
    ...
    const resetCurrentMode = () => {
        if (mode === 'stretch') {
            onPatch({
                frameSliceTop: sourceHeight > 10 ? Math.round(sourceHeight * 0.4) : 24,
                frameSliceRight: sourceWidth > 10 ? Math.round(sourceWidth * 0.4) : 28,
                frameSliceBottom: sourceHeight > 10 ? Math.round(sourceHeight * 0.4) : 24,
                frameSliceLeft: sourceWidth > 10 ? Math.round(sourceWidth * 0.4) : 28,
            });
            return;
        }
        onPatch({
    ...
    const displayValues = mode === 'stretch'
        ? [
            ['上', values.sliceTop], ['右', values.sliceRight], ['下', values.sliceBottom], ['左', values.sliceLeft],
        ]
        : [
            ['上', contentTop], ['右', visualContentRight], ['下', contentBottom], ['左', visualContentLeft],
        ];
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-3">
            <div className="mb-3 grid grid-cols-2 gap-2">
                <button
                    ...
                    className={`rounded-xl px-3 py-3 text-[11px] font-bold transition ${mode === 'stretch' ? 'bg-rose-500 text-white shadow-lg shadow-rose-200' : 'bg-rose-50 text-rose-500'}`}
                >
                    修改拉伸区域
                </button>
                <button
                    type="button"
                    onClick={() => { initializeContentRegion(); setMode('content'); }}
                    className={`rounded-xl px-3 py-3 text-[11px] font-bold transition ${mode === 'content' ? 'bg-sky-400 text-white shadow-lg shadow-sky-100' : 'bg-sky-50 text-sky-600'}`}
                >
                    修改内容区域
                </button>
            </div>
            ...
            <div className="mb-2 flex items-start justify-between gap-3">
                <div>
                    <b className="block text-[11px] text-slate-700">{mode === 'stretch' ? '拉伸区域' : '文字所在区域'}</b>
                    <span className="text-[9px] leading-relaxed text-slate-400">
                        {mode === 'stretch'
                            ? '四条红线围住的中间区域会被拉伸；把圆角、尾巴和小装饰留在线外。'
                            : '四条蓝线围住文字可用区域；文字不会再跑到尾巴、装饰或边框上。'}
                    </span>
                </div>
                <button type="button" onClick={resetCurrentMode} className="shrink-0 text-[9px] font-semibold text-slate-400">重置当前区域</button>
            </div>

            <div className="flex justify-center overflow-hidden rounded-xl border border-slate-100 bg-[linear-gradient(45deg,#f8fafc_25%,transparent_25%),linear-gradient(-45deg,#f8fafc_25%,transparent_25%),linear-gradient(45deg,transparent_75%,#f8fafc_75%),linear-gradient(-45deg,transparent_75%,#f8fafc_75%)] bg-[length:18px_18px] bg-[position:0_0,0_9px,9px_-9px,-9px_0px] p-2">
                <div ref={boxRef} className="relative inline-block max-w-full overflow-visible">
                    <img
                        src={url}
                        alt={mode === 'stretch' ? '气泡拉伸区域编辑预览' : '气泡内容区域编辑预览'}
                        draggable={false}
                        onLoad={(event) => {
                            const width = Math.max(1, event.currentTarget.naturalWidth);
                            const height = Math.max(1, event.currentTarget.naturalHeight);
                            setNaturalSize({ width, height });
                            if (style.frameSourceWidth !== width || style.frameSourceHeight !== height) {
                                onPatch({ frameSourceWidth: width, frameSourceHeight: height });
                            }
                        }}
                        className="block max-h-[240px] max-w-full select-none object-contain"
                        style={{ transform: style.frameFlipX ? 'scaleX(-1)' : undefined }}
                    />
                    {line('left', positions.left)}
                    {line('right', positions.right)}
                    {line('top', positions.top)}
                    {line('bottom', positions.bottom)}
                    <div
                        className="pointer-events-none absolute z-10 border border-dashed"
                        style={{
                            left: `${positions.left}%`,
                            right: `${100 - positions.right}%`,
                            top: `${positions.top}%`,
                            bottom: `${100 - positions.bottom}%`,
                            borderColor: mode === 'stretch' ? 'rgba(244,63,94,.9)' : 'rgba(56,189,248,.95)',
                            background: mode === 'stretch' ? 'rgba(244,63,94,.04)' : 'rgba(56,189,248,.06)',
                        }}
                    />
                </div>
            </div>
            ...
            <div className="mt-2 grid grid-cols-4 gap-1.5 text-center">
                {displayValues.map(([label, value]) => (
                    <div key={String(label)} className="rounded-lg bg-slate-50 px-1 py-1.5 text-[9px] text-slate-500">
                        <span>{label}</span><b className="ml-1 text-slate-700">{Math.round(Number(value))}px</b>
                    </div>
                ))}
            </div>

            <div className="mt-3 rounded-xl border border-sky-100 bg-[radial-gradient(#bae6fd_1px,transparent_1px)] bg-[length:14px_14px] px-3 py-6">
                <div className="mb-2 text-[9px] font-semibold text-sky-600">实际文字预览</div>
                <div className="flex justify-center">
                    <div
                        className="relative max-w-[94%] text-[14px] leading-relaxed"
                        style={{ ...bubbleFrameContainerStyle(style), color: style.textColor }}
                    >
                        <BubbleFrameLayer style={style} />
                        <span className="relative z-10">你说拯救世界　也包括我嘛</span>
                    </div>
                </div>
            </div>

            {style.frameFlipX && <p className="mt-2 text-[9px] leading-relaxed text-amber-500">当前开启水平镜像：左右的拉伸区域和内容区域会按镜像后的画面来编辑。</p>}
        </div>
    );
};
...
const ThemeMaker: React.FC = () => {
    ...
    const [assetUrlDraft, setAssetUrlDraft] = useState<Record<'bg' | 'frame' | 'deco' | 'avatarDeco', string>>({ bg: '', frame: '', deco: '', avatarDeco: '' });
    ...
    const activeFrameValues = resolveBubbleFrameValues(activeStyle);
    ...
    const updateStylePatch = (patch: Partial<BubbleStyle>) => {
        if (activeTab === 'css') return;
        updateTheme(prev => ({
            ...prev,
            [activeTab]: {
    ...
    const handleImageUpload = async (file: File, type: 'bg' | 'frame' | 'deco' | 'avatarDeco') => {
        try {
            ...
            if (type === 'bg') updateStyle('backgroundImage', stored);
                ...
                const size = await readImageSize(result);
                updateStylePatch({
                    frameImage: stored,
                    frameContentTop: undefined,
                    frameContentRight: undefined,
                    frameContentBottom: undefined,
                    frameContentLeft: undefined,
                    ...(size ? {
                        frameSourceWidth: size.width,
                        frameSourceHeight: size.height,
                        frameSliceTop: Math.round(size.height * 0.4),
                        frameSliceRight: Math.round(size.width * 0.4),
                        frameSliceBottom: Math.round(size.height * 0.4),
                        frameSliceLeft: Math.round(size.width * 0.4),
                    } : {}),
                });
            }
    ...
    const handleUrlApply = async (type: 'bg' | 'frame' | 'deco' | 'avatarDeco') => {
    ...
    const renderPreviewBubble = (mock: PreviewMockMessage, theme: ChatTheme, panel: 'A' | 'B', isLastInGroup: boolean) => {
        ...
        const frameActive = hasBubbleFrame(style);
        const hideTail = frameActive || shouldHideBubbleTail(style.tailMode, isLastInGroup);
        const containerStyle = {
            ...
            ...(frameActive ? bubbleFrameContainerStyle(style) : {}),
        };
                ...
                <div className={`relative group max-w-[78%] ${isUser ? 'mr-12' : 'ml-12'}`}>
                    ...
                    <div
                        className={isVoice
                            ...
                            : `relative ${frameActive ? '' : 'px-5 py-3 shadow-sm border border-black/5 '}text-sm overflow-visible ${isUser ? 'sully-bubble-user' : 'sully-bubble-ai'} ${isLastInGroup ? 'sully-bubble-group-last' : ''} ${hideTail ? 'sully-bubble-tail-hidden' : 'sully-bubble-tail-visible'} ${isActive ? 'ring-2 ring-primary/70' : ''}`}
                        style={isVoice ? undefined : containerStyle}
                    >
                        {!isVoice && frameActive && <BubbleFrameLayer style={style} />}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar pb-20">
                    ...
                    {activeTab !== 'css' && toolSection === 'base' && (
                        <div className="space-y-6 animate-fade-in"> 
                            ...
                            {/* Nine-slice Image Bubble */}
                            <div className="rounded-2xl border border-indigo-100 bg-indigo-50/45 p-4 space-y-4">
                                <div>
                                    <div className="flex items-center justify-between gap-3">
                                        <div>
                                            <h3 className="text-xs font-bold text-slate-600">自定义图片气泡 · 九宫格</h3>
                                            <p className="mt-1 text-[9px] leading-relaxed text-slate-400">上传完整 PNG / WebP 气泡。切片会保护四角和尾巴，中间区域随文字自动拉伸。</p>
                                        </div>
                                        {activeStyle.frameImage && <button type="button" onClick={() => updateStylePatch({ frameImage: undefined, frameSourceWidth: undefined, frameSourceHeight: undefined, frameContentTop: undefined, frameContentRight: undefined, frameContentBottom: undefined, frameContentLeft: undefined })} className="shrink-0 rounded-full bg-rose-50 px-2.5 py-1 text-[9px] font-semibold text-rose-500">移除</button>}
                                    </div>
                                </div>

                                <div onClick={() => frameInputRef.current?.click()} className="cursor-pointer group relative h-24 bg-white rounded-xl border-2 border-dashed border-indigo-100 flex flex-col items-center justify-center text-slate-400 overflow-hidden hover:border-primary/50 hover:text-primary transition-all">
                                    {activeStyle.frameImage ? (
                                        <>
                                            <TokenImg value={activeStyle.frameImage} className="absolute inset-2 w-[calc(100%_-_1rem)] h-[calc(100%_-_1rem)] object-contain opacity-80" />
                                            <span className="relative z-10 text-[10px] bg-white/85 px-2 py-1 rounded shadow-sm font-bold">更换图片气泡</span>
                                        </>
                                    ) : <span className="text-xs font-bold">+ 上传完整气泡 PNG / WebP</span>}
                                    <input type="file" ref={frameInputRef} className="hidden" accept="image/png,image/webp,image/jpeg" onChange={(e) => e.target.files?.[0] && handleImageUpload(e.target.files[0], 'frame')} />
                                </div>

                                <div className="flex gap-2">
                                    <input
                                        type="url"
                                        value={assetUrlDraft.frame}
                                        onChange={(e) => setAssetUrlDraft(prev => ({ ...prev, frame: e.target.value }))}
                                        placeholder="或粘贴完整气泡图床 URL"
                                    ...
                                    />
                                    <button onClick={() => handleUrlApply('frame')} className="px-3 py-2 rounded-lg bg-white text-xs font-semibold text-slate-600 border border-slate-200 hover:bg-slate-50">应用</button>
                                </div>

                                {activeStyle.frameImage && (
                                    <div className="space-y-4">
                                        <BubbleNineSliceGuide style={activeStyle} onPatch={updateStylePatch} />
                                        <details className="rounded-xl border border-slate-100 bg-white/70 p-3">
                                            <summary className="cursor-pointer list-none text-[10px] font-semibold text-slate-500">数值微调（可选） <span className="text-slate-300">· 拖线不好抓时再用</span></summary>
                                            <div className="mt-3 space-y-4">
                                                <div className="rounded-xl bg-white/80 p-3">
                                            <div className="mb-3 flex items-center justify-between">
                                                <div>
                                                    <b className="block text-[10px] text-slate-600">① 九宫格切片</b>
                                                    <span className="text-[9px] text-slate-400">把四条线调到圆角/尾巴的内侧；值越大，保留区域越宽。</span>
                                                </div>
                                                <button type="button" onClick={() => {
                                                    ...
                                                    updateTheme(prev => ({
                                                        ...
                                                        [activeTab]: {
                                                            ...prev[activeTab],
                                                            frameSliceTop: activeFrameValues.sourceHeight > 10 ? Math.round(activeFrameValues.sourceHeight * 0.4) : 24,
                                                            frameSliceRight: activeFrameValues.sourceWidth > 10 ? Math.round(activeFrameValues.sourceWidth * 0.4) : 28,
                                                            frameSliceBottom: activeFrameValues.sourceHeight > 10 ? Math.round(activeFrameValues.sourceHeight * 0.4) : 24,
                                                            frameSliceLeft: activeFrameValues.sourceWidth > 10 ? Math.round(activeFrameValues.sourceWidth * 0.4) : 28,
                                                        }
                                                    }));
                                                }} className="text-[9px] text-indigo-500">恢复推荐</button>
                                            </div>
                                            <div className="grid grid-cols-2 gap-3">
                                                {([
                                                    ['frameSliceTop','上',activeFrameValues.sliceTop],
                                                    ['frameSliceRight','右',activeFrameValues.sliceRight],
                                                    ['frameSliceBottom','下',activeFrameValues.sliceBottom],
                                                    ['frameSliceLeft','左',activeFrameValues.sliceLeft],
                                                ] as const).map(([key,label,value]) => (
                                                    <label key={key} className="text-[9px] text-slate-500">
                                                        <span className="mb-1 flex justify-between"><span>{label}</span><b>{Math.round(value)}px</b></span>
                                                        <input type="range" min="0" max="500" step="1" value={Math.min(500, value)} onChange={(e) => updateStyle(key, parseInt(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full accent-primary" />
                                                    </label>
                                                ))}
                                            </div>
                                        </div>
                                        ...
                                        <div className="rounded-xl bg-white/80 p-3">
                                            <div className="mb-3">
                                                <b className="block text-[10px] text-slate-600">② 文字所在区域</b>
                                                <span className="text-[9px] text-slate-400">这里和蓝线完全同步：数值表示文字区域离原图四边的距离。</span>
                                            </div>
                                            <div className="grid grid-cols-2 gap-3">
                                                {([
                                                    ['frameContentTop','上',activeFrameValues.contentTop ?? Math.round(activeFrameValues.sliceTop * 0.68),activeFrameValues.sliceTop],
                                                    ['frameContentRight','右',activeFrameValues.contentRight ?? Math.round(activeFrameValues.sliceRight * 0.72),activeFrameValues.sliceRight],
                                                    ['frameContentBottom','下',activeFrameValues.contentBottom ?? Math.round(activeFrameValues.sliceBottom * 0.68),activeFrameValues.sliceBottom],
                                                    ['frameContentLeft','左',activeFrameValues.contentLeft ?? Math.round(activeFrameValues.sliceLeft * 0.72),activeFrameValues.sliceLeft],
                                                ] as const).map(([key,label,value,maxValue]) => (
                                                    <label key={key} className="text-[9px] text-slate-500">
                                                        <span className="mb-1 flex justify-between"><span>{label}</span><b>{Math.round(value)}px</b></span>
                                                        <input type="range" min="0" max={Math.max(1, Math.round(maxValue))} step="1" value={Math.min(Math.max(1, Math.round(maxValue)), value)} onChange={(e) => updateStyle(key, parseInt(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full accent-primary" />
                                                    </label>
                                                ))}
                                            </div>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-3 rounded-xl bg-white/80 p-3">
                                            <label className="text-[9px] text-slate-500">
                                                <span className="mb-1 flex justify-between"><span>边缘缩放</span><b>{activeFrameValues.scale.toFixed(2)}×</b></span>
                                                <input type="range" min="0.25" max="3" step="0.05" value={activeFrameValues.scale} onChange={(e) => updateStyle('frameScale', parseFloat(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full accent-primary" />
                                            </label>
                                            <label className="text-[9px] text-slate-500">
                                                <span className="mb-1 flex justify-between"><span>皮肤透明度</span><b>{Math.round(activeFrameValues.opacity*100)}%</b></span>
                                                <input type="range" min="0" max="1" step="0.05" value={activeFrameValues.opacity} onChange={(e) => updateStyle('frameOpacity', parseFloat(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full accent-primary" />
                                            </label>
                                            <label className="col-span-2 text-[9px] text-slate-500">
                                                <span className="mb-1 flex justify-between"><span>最小高度</span><b>{Math.round(activeFrameValues.minHeight)}px</b></span>
                                                <input type="range" min="0" max="100" step="1" value={activeFrameValues.minHeight} onChange={(e) => updateStyle('frameMinHeight', parseInt(e.target.value))} className="w-full h-1.5 bg-slate-200 rounded-full accent-primary" />
                                            </label>
                                        </div>

                                        <div>
                                            <div className="mb-2 text-[9px] font-semibold text-slate-500">边缘拉伸方式</div>
                                            <div className="grid grid-cols-2 gap-2">
                                                {([['stretch','拉伸'],['round','平铺适配']] as const).map(([mode,label]) => (
                                                    <button key={mode} type="button" onClick={() => updateStyle('frameRepeat', mode)} className={`rounded-xl border py-2 text-[10px] font-semibold ${activeFrameValues.repeat===mode?'border-primary bg-primary/10 text-primary':'border-slate-200 bg-white text-slate-500'}`}>{label}</button>
                                                ))}
                                            </div>
                                            <button type="button" onClick={() => updateStyle('frameFlipX', !activeStyle.frameFlipX)} className={`mt-2 w-full rounded-xl border py-2 text-[10px] font-semibold ${activeStyle.frameFlipX?'border-primary bg-primary/10 text-primary':'border-slate-200 bg-white text-slate-500'}`}>水平镜像图片皮肤{activeStyle.frameFlipX?' · 已开启':''}</button>
                                            <p className="mt-2 text-[9px] leading-relaxed text-slate-400">图片气泡启用后，会自动关闭原 CSS 小尾巴，避免出现两条尾巴；需要把右尾巴素材换成左尾巴时可直接镜像，文字不会翻转。底纹、贴纸和头像框仍可继续叠加。</p>
                                        </div>
                                    </div>
                                )}
                            </div>
