...
export interface OSTheme {
  ...
  /** 顶部时间 / Wi‑Fi / 电量整体上下微调，单位 px（负数上移、正数下移）。 */
  statusBarOffsetY?: number;
  /** 顶部时间 / Wi‑Fi / 电量的独立颜色；未设置时继续跟随 contentColor。 */
  ...
  /** 可视化聊天素材槽：顶栏、输入栏、按钮等可上传图片并调位置/透明度。 */
  chatAssetDecorations?: ChatAssetDecorations;
  /** 聊天顶部 / 输入区控件的自由移动、显隐与图标换肤。 */
  chatControlLayout?: ChatControlLayout;
...
export interface ImageGenApiConfig {
  ...
  /** 用户明确要求看角色/要照片时，无条件进入聊天生图链路。缺省开启。 */
  forcePhotoOnExplicitRequest?: boolean;
  /** 普通聊天里允许角色根据上下文主动分享照片；缺省开启。 */
  allowProactiveChatPhotos?: boolean;
...
export interface CharacterBrainApiConfig {
  /** 关闭=只用本地活人感规则；follow=复用主聊天 API；independent=使用独立便宜模型。 */
  mode: 'off' | 'follow' | 'independent';
  baseUrl?: string;
  apiKey?: string;
  model?: string;
  /** 0-100：只影响活人感提示强度，不改变角色固定人设。 */
  liveliness?: number;
}
...
export interface APIConfig {
  baseUrl: string;
  apiKey: string;
  /** 独立翻译 API；只负责点击“翻译”时的翻译请求，不占用主聊天模型。 */
  translationApi?: TranslationApiConfig;
  /** 角色大脑：生活状态/主动性/聊天节奏可与主聊天 API 分离。 */
  characterBrainApi?: CharacterBrainApiConfig;
...
export interface BubbleStyle {
    ...
    /** 九宫格图片气泡。存在时优先于普通气泡底色/边框，四角与尾巴可通过切片保持不变形。 */
    frameImage?: string;
    frameSliceTop?: number;
    frameSliceRight?: number;
    frameSliceBottom?: number;
    frameSliceLeft?: number;
    /** 原始图片尺寸，用来把高分辨率素材的切片坐标换算成合理的屏幕边框厚度。 */
    frameSourceWidth?: number;
    frameSourceHeight?: number;
    /** 文字内容区：相对原图四边的安全内缩距离（原图像素）。用于可视化规定文字所在区域。 */
    frameContentTop?: number;
    frameContentRight?: number;
    frameContentBottom?: number;
    frameContentLeft?: number;
    ...
    frameScale?: number;
    frameOpacity?: number;
    frameRepeat?: 'stretch' | 'round';
    frameMinHeight?: number;
    /** 仅镜像图片皮肤本身，文字不翻转；适合把右尾巴素材复用成左尾巴。 */
    frameFlipX?: boolean;
    borderRadius: number;
...
export type ChatAssetFit = 'cover' | 'contain' | 'stretch' | 'tile';

export interface ChatAssetSlot {
    image?: string;
    opacity?: number;
    x?: number;
    y?: number;
...
export interface ChatAssetDecorations {
    statusBarBackground?: ChatAssetSlot;
    headerBackground?: ChatAssetSlot;
    headerDecoration?: ChatAssetSlot;
    inputBackground?: ChatAssetSlot;
    inputDecoration?: ChatAssetSlot;
    sendButton?: ChatAssetSlot;
}

export type ChatControlKey =
    | 'headerAvatar'
    | 'headerName'
    | 'onlineStatus'
    | 'modelSwitcher'
...
export interface ChatControlSlot {
    /** 相对原布局的水平偏移，单位 px。 */
    x?: number;
    /** 相对原布局的垂直偏移，单位 px。 */
    y?: number;
    ...
    /** 只锁编辑器里的拖动，不影响按钮点击。 */
    locked?: boolean;
    /** 按钮默认视觉图片；文字类 / 头像类控件可只移动，不使用该字段。 */
    icon?: string;
    /** 闪电停止态 / 录音停止态 / 发送按钮生成态的第二张视觉图片。 */
    activeIcon?: string;
}

export interface ChatControlLayout {
    /** 关闭时完全沿用原布局；开启后才应用各控件偏移与图片。 */
    enabled?: boolean;
    headerAvatar?: ChatControlSlot;
    headerName?: ChatControlSlot;
    onlineStatus?: ChatControlSlot;
    modelSwitcher?: ChatControlSlot;
    activityStatus?: ChatControlSlot;
    tokenUsage?: ChatControlSlot;
    backButton?: ChatControlSlot;
    triggerButton?: ChatControlSlot;
    actionsButton?: ChatControlSlot;
    voiceButton?: ChatControlSlot;
    emojiButton?: ChatControlSlot;
    sendButton?: ChatControlSlot;
}
...
export interface CharacterProfile {
  ...
  /** 当前角色专属聊天素材；未设置的槽位继续跟随全局。 */
  chatAssetDecorations?: ChatAssetDecorations;
  /** 当前角色专属的聊天控件自由布局；未设置字段继续跟随全局。 */
  chatControlLayout?: ChatControlLayout;
