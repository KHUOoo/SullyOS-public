...
import { applyAssistantPostProcessing, needsAutomaticBilingualTranslation, PostProcessCtx, splitSARChatSurfaceBubbles, XhsCaches } from './applyAssistantPostProcessing';
import { DB } from './db';
...
describe('外语翻译代码层兜底', () => {
    it('识别未翻译外语，已带双语标记/纯 URL 不重复触发', () => {
        expect(needsAutomaticBilingualTranslation('Where are you?')).toBe(true);
        expect(needsAutomaticBilingualTranslation('你认真的？ Seriously?')).toBe(true);
        expect(needsAutomaticBilingualTranslation('どこにいるの？')).toBe(true);
        expect(needsAutomaticBilingualTranslation('Where are you?\n%%BILINGUAL%%\n你在哪儿？')).toBe(false);
        expect(needsAutomaticBilingualTranslation('https://example.com/Draco')).toBe(false);
        expect(needsAutomaticBilingualTranslation('我在用 iPhone 拍照')).toBe(false);
        expect(needsAutomaticBilingualTranslation('今天早点睡。')).toBe(false);
    });

    it('模型漏翻译时，在落库前自动补成 %%BILINGUAL%% 气泡', async () => {
        const charId = `c-auto-bi-${Date.now()}`;
        const fetchMock = vi.fn(async () => new Response(JSON.stringify({
            choices: [{ message: { content: '你在哪儿？' } }],
        }), {
            status: 200,
        ...
        try {
            ...
            ctx.api.baseUrl = 'https://chat.example/v1';
            ctx.api.effectiveApi = { baseUrl: 'https://chat.example/v1', apiKey: 'key', model: 'model' } as any;

            await applyAssistantPostProcessing('Where are you?', ctx);

            const msgs = (await DB.getRecentMessagesByCharId(charId, 20)).filter(m => m.role === 'assistant' && m.type === 'text');
            expect(msgs).toHaveLength(1);
            expect(msgs[0].content).toBe('Where are you?\n%%BILINGUAL%%\n你在哪儿？');
    ...
    it('已有 <翻译> 标签时不再调用翻译接口', async () => {
        const charId = `c-auto-bi-existing-${Date.now()}`;
        const fetchMock = vi.fn();
        vi.stubGlobal('fetch', fetchMock);
        try {
            const ctx = makeCtx(charId, []);
            ctx.instantRender = true;
            await applyAssistantPostProcessing('<翻译><原文>Fine.</原文><译文>行。</译文></翻译>', ctx);
            const msgs = (await DB.getRecentMessagesByCharId(charId, 20)).filter(m => m.role === 'assistant' && m.type === 'text');
            expect(msgs[0].content).toBe('Fine.\n%%BILINGUAL%%\n行。');
...
describe('renderAndPersist 角色发图意图', () => {
    it('隐藏伪照片长描述，只保留短台词并把生图意图写入 metadata', async () => {
        const charId = `c-image-intent-${Date.now()}`;
        const ctx = makeCtx(charId, []);
        ctx.instantRender = true;

        await applyAssistantPostProcessing(
            '好吧。[你发送了一张照片：随意半躺在宿舍四柱床上，手里捏着翻了一半的书，灯光昏暗偏暖。]\n现在开心了。',
            ctx,
        );

        const texts = (await DB.getRecentMessagesByCharId(charId, 20))
            .filter(m => m.role === 'assistant' && m.type === 'text');
        expect(texts.map(m => m.content)).toEqual(['好吧。', '现在开心了。']);
        expect(texts[0].metadata?.assistantImageIntent?.status).toBe('pending');
        expect(texts[0].metadata?.assistantImageIntent?.source).toBe('legacy_photo_text');
        expect(texts[0].metadata?.assistantImageIntent?.scene).toContain('宿舍四柱床');
        expect(texts[0].content).not.toContain('你发送了一张照片');
    }, 20000);

    it('纯 SEND_IMAGE 指令不会把 prompt 显示出来，会留下极短承载气泡', async () => {
        const charId = `c-image-directive-${Date.now()}`;
        const ctx = makeCtx(charId, []);
        ctx.instantRender = true;

        await applyAssistantPostProcessing(
            '[[SEND_IMAGE: 窗边随手自拍，头发微乱，自然光。]]',
            ctx,
        );
        ...
        expect(texts[0].content).toBe('给你。');
        expect(texts[0].metadata?.assistantImageIntent?.scene).toContain('窗边随手自拍');
    }, 20000);
});
