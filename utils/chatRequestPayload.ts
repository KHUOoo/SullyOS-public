...
<译文>今天做什么？</译文>
    ...
    // ── 4b. 真实发图协议 ─────────────────────────────────
    // ChatApp 里角色若真的决定发图片，必须把画面描述放进机器指令，不能把“发送了一张照片：长描述”
    // 当成普通聊天文字。后处理会隐藏指令，Chat.tsx 再复用现有深图 API 真正生成图片气泡。
    if (input.recallEntryPoint === 'chat_app') {
        systemPrompt += `

[CRITICAL: 真实发图协议]
当你真的决定现在给用户发送一张照片/自拍/房间/食物/穿搭/环境图时：
- 只用 [[SEND_IMAGE: 具体画面描述]] 表达“发送图片”的动作。
- SEND_IMAGE 内写给图片生成模型看的具体画面，包括主体、动作、环境、光线、镜头感；不要写解释。
- SEND_IMAGE 之外最多保留 0~2 句很短的正常聊天台词。
- 绝对不要输出“[你发送了一张照片：……]”“【发送图片：……】”或其他把长图片描述伪装成聊天正文的写法。
- 如果你并不是真的要发图，就不要输出 SEND_IMAGE。
- 若同时处于双语模式，SEND_IMAGE 放在所有 <翻译> 标签外；普通台词仍按双语规则输出。
示例：
[[SEND_IMAGE: 夜晚宿舍里随手拍的一张自拍，角色半躺在床上，头发微乱，暖黄灯光，旧手机抓拍感。]]
给你。`;
    }
