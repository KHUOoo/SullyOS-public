...
import { extractAssistantImageIntent } from './assistantImageIntent';
...
/**
 * 代码层双语兜底：
 * 世界书/提示词只能“要求”模型翻译，模型仍可能漏格式；这里在真正落库前再检查一次。
 * 只检查最终可见文本，不碰 URL、控制标签、HTML 标签；已经有 %%BILINGUAL%% 或
 * <翻译><原文>...<译文>...</翻译> 的内容直接放行，避免重复翻译。
 */
const AUTO_BILINGUAL_MARKER = '%%BILINGUAL%%';
const AUTO_BILINGUAL_TAG_RE = /<翻译>\s*<原文>[\s\S]*?<\/原文>\s*<译文>[\s\S]*?<\/译文>\s*<\/翻译>/;
const AUTO_LATIN_SCRIPT_RE = /[A-Za-zÀ-ÖØ-öø-ÿĀ-ž]/u;
const AUTO_NON_LATIN_FOREIGN_SCRIPT_RE = /[Α-ωА-Яа-яЁё\u0600-\u06ff\u0e00-\u0e7f\u3040-\u30ff\u31f0-\u31ff\uac00-\ud7af]/u;
const AUTO_HAN_RE = /[\u3400-\u9fff]/u;
const AUTO_LATIN_TOKEN_RE = /[A-Za-zÀ-ÖØ-öø-ÿĀ-ž]+(?:['’\-][A-Za-zÀ-ÖØ-öø-ÿĀ-ž]+)*/gu;

export const needsAutomaticBilingualTranslation = (value: string): boolean => {
    const text = String(value || '').trim();
    if (!text) return false;
    if (text.includes(AUTO_BILINGUAL_MARKER) || AUTO_BILINGUAL_TAG_RE.test(text)) return false;
    if (/^\[(?:表情|表情包|图片|语音|HTML\s*卡片)[：:]/u.test(text)) return false;

    const visibleProbe = text
        .replace(/https?:\/\/\S+/gi, ' ')
        .replace(/\bwww\.\S+/gi, ' ')
    ...
    if (!visibleProbe) return false;
    if (AUTO_NON_LATIN_FOREIGN_SCRIPT_RE.test(visibleProbe)) return true;
    if (!AUTO_LATIN_SCRIPT_RE.test(visibleProbe)) return false;

    // 纯外语消息（哪怕只有 Fine / No / Hello 一个词）一定翻译。
    if (!AUTO_HAN_RE.test(visibleProbe)) return true;

    // 中英混合时避免把 iPhone / Draco / WiFi 这类单个专名误判成“外语句子”。
    // 两个以上外语词，或单个外语词本身带句末标点，才视为真正的外语表达。
    const latinTokens = visibleProbe.match(AUTO_LATIN_TOKEN_RE) || [];
    if (latinTokens.length >= 2) return true;
    if (latinTokens.length === 1) {
        const escaped = latinTokens[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        return new RegExp(`${escaped}\\s*[!?！？。,.，…]+`, 'u').test(visibleProbe);
    }
    return false;
};
/**
...
export async function applyAssistantPostProcessing(
    ...
    const translateAssistantTextIfNeeded = async (value: string): Promise<string> => {
        const text = String(value || '').trim();
        if (!needsAutomaticBilingualTranslation(text)) return value;

        const endpoint = `${baseUrl.replace(/\/+$/, '')}/chat/completions`;
        try {
            const translationData = await safeFetchJson(endpoint, {
                method: 'POST',
                headers,
                body: JSON.stringify({
                    ...
                    stream: false,
                    temperature: 0,
                    max_tokens: Math.min(1600, Math.max(256, text.length * 4)),
                    messages: [
                        {
                            role: 'system',
                            content: '你是聊天消息翻译器。把用户提供的整段消息准确翻译成简体中文，保留语气、称呼、标点与含义。不要解释，不要加“翻译：”，不要输出原文，不要输出 %%BILINGUAL%%，只输出中文译文。若原文中已经有中文，保留中文并只把其中的外语部分翻成中文。',
                        },
                        { role: 'user', content: text },
                    ],
                }),
            }, 2, 0, { ...apiLogMeta, purpose: '外语翻译兜底' });

            let translated = String(translationData?.choices?.[0]?.message?.content || '').trim();
            translated = translated
                .replace(/^\s*(?:译文|翻译)\s*[：:]\s*/u, '')
                .replace(/^```(?:text|txt)?\s*/i, '')
                .replace(/```\s*$/i, '')
                .trim();

            if (!translated) throw new Error('翻译接口没有返回译文');
            return `${value}\n${AUTO_BILINGUAL_MARKER}\n${translated}`;
        } catch (error) {
            console.warn('[bilingual-guard] automatic translation failed', {
                charId: char.id,
                error,
            });
            return `${value}\n${AUTO_BILINGUAL_MARKER}\n（翻译暂时失败）`;
    ...
    const persistMessage: typeof DB.saveMessage = async (msg) => {
        ...
        if (
            ...
            && needsAutomaticBilingualTranslation(msg.content)
        ) {
            nextMsg = { ...msg, content: await translateAssistantTextIfNeeded(msg.content) };
        }
        return DB.saveMessage(messageTimestamp != null ? { ...nextMsg, timestamp: messageTimestamp } : nextMsg);
    };
    const {
        setMessages,
        addToast,
    ...
    const renderAndPersist = async (rawContent: string, firstThinkingChain: string | null): Promise<void> => {
        const parsedImageIntent = extractAssistantImageIntent(rawContent);
        let pendingAssistantImageIntent: any = parsedImageIntent
            ? {
                scene: parsedImageIntent.scene,
                source: parsedImageIntent.source,
                status: 'pending',
                detectedAt: Date.now(),
            }
            : null;
        // 图片描述只给生图 API 使用，不再作为几十上百字的聊天正文展示。
        // 如果模型只输出了发图指令，留一个极短的自然气泡承载 metadata，后续 Chat effect 会真正发图。
        if (parsedImageIntent) {
            rawContent = parsedImageIntent.cleanedText || '给你。';
        }
        ...
        const takeTextMeta = (base: any, canonicalChunk?: string): any => {
            const inherited = takeMeta(base, canonicalChunk);
            if (!pendingAssistantImageIntent) return inherited;
            const merged = {
                ...(inherited || {}),
                assistantImageIntent: pendingAssistantImageIntent,
            };
            pendingAssistantImageIntent = null;
            return merged;
        };
        ...
        if (hasTranslationTags) {
            ...
            const renderPlainSegment = async (segment: string): Promise<void> => {
                for (const part of ChatParser.splitResponse(segment)) {
                    ...
                    for (const chunk of chunks) {
                        ...
                        await persistMessage({ charId: char.id, role: 'assistant', type: 'text', content: chunk, replyTo: replyData, metadata: takeTextMeta(mcdInheritMeta, chunk) } as any);
                        setMessages(await DB.getRecentMessagesByCharId(char.id, 200));
                        globalMsgIndex++;
                    }
                }
            ...
            while ((tagMatch = tagPattern.exec(content)) !== null) {
                ...
                if (originalText || translatedText) {
                    ...
                    await persistMessage({ charId: char.id, role: 'assistant', type: 'text', content: biContent, replyTo: replyData, metadata: takeTextMeta(mcdInheritMeta, biContent) } as any);
                    setMessages(await DB.getRecentMessagesByCharId(char.id, 200));
                    globalMsgIndex++;
                }
                ...
                if (part.type === 'emoji') {
                    ...
                    for (let i = 0; i < allChunks.length; i++) {
                        ...
                        if (ChatParser.hasDisplayContent(chunk)) {
                            ...
                            if (cleanChunk) {
                                await persistMessage({ charId: char.id, role: 'assistant', type: 'text', content: cleanChunk, replyTo: replyData, metadata: takeTextMeta(mcdInheritMeta, cleanChunk) } as any);
                                setMessages(await DB.getRecentMessagesByCharId(char.id, 200));
                                globalMsgIndex++;
                                chunkSaved = true;
                            }
