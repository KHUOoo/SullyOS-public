...
const StatusBar: React.FC = () => {
  ...
  const textColor = theme.statusBarColor?.trim() || theme.contentColor || '#ffffff';
  const statusBarOffsetY = Number.isFinite(theme.statusBarOffsetY) ? Math.max(-24, Math.min(40, Number(theme.statusBarOffsetY))) : 0;
  const statusBarContentStyle: React.CSSProperties = statusBarOffsetY ? { transform: `translateY(${statusBarOffsetY}px)` } : {};
  ...
  return (
    <>
      ...
      {!hideOsStatusBar && (
      <div
          className="sully-chat-statusbar w-full flex justify-between items-start px-6 text-[11px] font-bold z-50 absolute top-0 left-0 bg-transparent transition-colors duration-500 select-none pointer-events-none"
          style={{
              color: textColor,
      ...
      >
        <div className="w-1/3 pl-2 flex items-center gap-2 pointer-events-auto" style={statusBarContentStyle}>
          <span>{format(virtualTime.hours)}:{format(virtualTime.minutes)}</span>
        </div>
        <div className="w-1/3 flex justify-center" style={statusBarContentStyle}>
          {/* Notch Area spacer */}
        </div>
        <div className="w-1/3 flex justify-end gap-1.5 items-center pr-2" style={statusBarContentStyle}>
