...
interface ChatHeaderShellProps {
    ...
    isTyping: boolean;
    /** 角色照片链路状态。非 idle 时优先于普通“正在输入”。 */
    photoActivityState?: 'idle' | 'generating_photo' | 'sending_photo' | 'failed';
...
type ActivityPosition = { x: number; y: number };
const DEFAULT_ACTIVITY_POSITION: ActivityPosition = { x: 0.5, y: 0.82 };
const clampActivity = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));
...
const ChatHeaderShell: React.FC<ChatHeaderShellProps> = ({
    ...
    const [activityPosition, setActivityPosition] = useState<ActivityPosition>(DEFAULT_ACTIVITY_POSITION);
    const activityPositionRef = useRef<ActivityPosition>(DEFAULT_ACTIVITY_POSITION);
    const activityDragRef = useRef<{ pointerId: number; offsetX: number; offsetY: number } | null>(null);
    const activityStorageKey = `sullyos.chat.activityBadgePosition.${activeCharacter.id}`;
    ...
    useEffect(() => {
        let next = DEFAULT_ACTIVITY_POSITION;
        try {
        ...
        activityPositionRef.current = next;
        setActivityPosition(next);
    }, [activityStorageKey]);

    const persistActivityPosition = (position: ActivityPosition) => {
        try {
            localStorage.setItem(activityStorageKey, JSON.stringify(position));
        } catch { /* storage may be unavailable */ }
    };

    const handleActivityPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
        const badge = activityBadgeRef.current;
        if (!badge) return;
    ...
    const handleActivityPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
        ...
        const badge = activityBadgeRef.current;
        if (!drag || drag.pointerId !== event.pointerId || !header || !badge) return;
        const headerRect = header.getBoundingClientRect();
        const badgeRect = badge.getBoundingClientRect();
        const halfW = badgeRect.width / 2;
        const halfH = badgeRect.height / 2;
        const centerX = clampActivity(event.clientX - drag.offsetX - headerRect.left, halfW, Math.max(halfW, headerRect.width - halfW));
        const centerY = clampActivity(event.clientY - drag.offsetY - headerRect.top, halfH, Math.max(halfH, headerRect.height - halfH));
        const next = {
            x: headerRect.width > 0 ? centerX / headerRect.width : DEFAULT_ACTIVITY_POSITION.x,
            y: headerRect.height > 0 ? centerY / headerRect.height : DEFAULT_ACTIVITY_POSITION.y,
        };
        activityPositionRef.current = next;
        setActivityPosition(next);
        event.preventDefault();
    ...
    const finishActivityDrag = (event: React.PointerEvent<HTMLDivElement>) => {
        const drag = activityDragRef.current;
        if (!drag || drag.pointerId !== event.pointerId) return;
        activityDragRef.current = null;
        persistActivityPosition(activityPositionRef.current);
        try { event.currentTarget.releasePointerCapture?.(event.pointerId); } catch { /* ignore */ }
        event.preventDefault();
        event.stopPropagation();
    };

    const resetActivityPosition = (event: React.MouseEvent<HTMLDivElement>) => {
        activityPositionRef.current = DEFAULT_ACTIVITY_POSITION;
        setActivityPosition(DEFAULT_ACTIVITY_POSITION);
        persistActivityPosition(DEFAULT_ACTIVITY_POSITION);
        event.preventDefault();
        event.stopPropagation();
    };
    ...
    const onlineStatusNode = statusStyle === 'pill' ? (
            <div className={`sully-chat-online-status text-[9px] px-1.5 py-0.5 rounded-full font-semibold border ${isDarkHeader ? 'bg-emerald-500/20 text-emerald-200 border-emerald-400/20' : isPixelHeader ? 'bg-[#fff7ed] text-[#8f674a] border-[#8f674a]/25' : 'bg-emerald-50 text-emerald-500 border-emerald-100'}`}>
                {statusText ?? 'online'}
            </div>
        ) : statusStyle === 'dot' ? (
            <div className={`sully-chat-online-status flex items-center gap-1 text-[10px] ${secondaryTextClass}`}>
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                <span>{statusText ?? 'Online'}</span>
            </div>
        ) : (
            <div className={`sully-chat-online-status text-[10px] uppercase ${secondaryTextClass}`}>{statusText ?? 'Online'}</div>
        );

    const effectiveActivityPhase =
        photoActivityState !== 'idle'
            ? photoActivityState
            : isTyping
              ? 'typing'
              : null;
    const activityLabel = effectiveActivityPhase === 'typing'
        ? '正在输入…'
        : effectiveActivityPhase === 'generating_photo'
          ? '正在生成照片…'
          : effectiveActivityPhase === 'sending_photo'
            ? '正在发送照片…'
            : effectiveActivityPhase === 'failed'
              ? '照片发送失败'
              : '';
    const activityBadgeNode = effectiveActivityPhase && !selectionMode ? (
        <div
            ref={activityBadgeRef}
            className={`sully-chat-activity-badge absolute z-[70] inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold shadow-sm select-none cursor-grab active:cursor-grabbing ${effectiveActivityPhase === 'failed'
                ? (isDarkHeader ? 'border-rose-400/25 bg-rose-500/15 text-rose-200' : 'border-rose-200 bg-white/95 text-rose-500')
                : (isDarkHeader ? 'border-white/10 bg-slate-900/88 text-slate-200' : isPixelHeader ? 'border-[#8f674a]/25 bg-[#fff7ed]/95 text-[#8f674a]' : acnh ? 'border-[#e6dab4] bg-[#fbf4de]/95 text-[#6b5a3e]' : 'border-slate-200 bg-white/95 text-slate-600')
            }`}
            style={{
                left: `${activityPosition.x * 100}%`,
                top: `${activityPosition.y * 100}%`,
                transform: 'translate(-50%, -50%)',
                touchAction: 'none',
                maxWidth: '12rem',
                boxShadow: '0 4px 14px -6px rgba(15,23,42,0.28)',
            }}
            title="拖动可调整状态位置，双击恢复默认位置"
            onPointerDown={handleActivityPointerDown}
            onPointerMove={handleActivityPointerMove}
            onPointerUp={finishActivityDrag}
            onPointerCancel={finishActivityDrag}
            onDoubleClick={resetActivityPosition}
        >
            {effectiveActivityPhase === 'failed' ? (
                <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-rose-400" />
            ) : (
                <span className={`h-2.5 w-2.5 shrink-0 rounded-full border-2 animate-spin ${isDarkHeader ? 'border-slate-600 border-t-sky-300' : 'border-slate-200 border-t-emerald-500'}`} />
            )}
            <span className="truncate whitespace-nowrap">{activityLabel}</span>
        </div>
    ) : null;
    ...
    const floatingStatusNodes = (isInstantSending || isEmotionEvaluating || isMemoryPalaceProcessing) ? (
    ...
    // Token 永远脱离 flex / 文档流，数字变长或显示/隐藏都不能撑高顶栏。
    const tokenUsageNode = lastTokenUsage ? (
        <div className="pointer-events-none absolute right-12 top-[58%] z-[65]">
            <div
                className={`sully-chat-token text-[9px] px-1.5 py-0.5 rounded-md font-mono border whitespace-nowrap ${isDarkHeader ? 'bg-slate-800 text-slate-300 border-white/10' : isPixelHeader ? 'bg-[#fff7ed] text-[#8f674a] border-[#8f674a]/20' : 'bg-slate-100/95 text-slate-400 border-slate-200'}`}
    ...
    return (
        ...
        <div ref={headerRef} className={`sully-chat-header ${headerDensityClass} flex items-center relative ${headerToneClass}`} style={headerSafeStyle}>
            ...
            {selectionMode ? (
                ...
                <div className="relative w-full min-h-[56px] flex items-end justify-center">
                    ...
                    {!hideTrigger && <button onClick={onTriggerAI} className={`sully-chat-trigger absolute right-0 bottom-2 p-2 ${actionButtonClass}`} data-control-state={triggerIcon === 'stop' ? 'active' : 'default'} title={triggerIcon === 'stop' ? '停止生成' : '触发 AI'}>
                        {triggerIconNode}
                    </button>}
                    {extraAction && (
                ...
                <div className="flex items-center gap-3 w-full">
                    ...
                    {!hideTrigger && <button onClick={onTriggerAI} className={`sully-chat-trigger p-2 ${extraAction ? '' : 'ml-auto'} ${actionButtonClass}`} data-control-state={triggerIcon === 'stop' ? 'active' : 'default'} title={triggerIcon === 'stop' ? '停止生成' : '触发 AI'}>
                        {triggerIconNode}
                    </button>}
                </div>
            )}
