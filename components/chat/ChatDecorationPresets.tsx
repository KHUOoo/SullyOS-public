...
import type {ChatAssetDecorations,ChatControlKey,ChatControlLayout,ChatTheme} from '../../types';
type ImageUse='background'|'user'|'ai'|'frameUser'|'frameAi'|'statusBarBackground'|'headerBackground'|'headerDecoration'|'inputBackground'|'inputDecoration'|'sendButton'|'backIcon'|'triggerIcon'|'triggerActiveIcon'|'actionsIcon'|'voiceIcon'|'voiceActiveIcon'|'emojiIcon'|'sendIcon'|'sendActiveIcon';
...
export default function ChatDecorationPresets({target,scope,currentBubble,exportCurrent,onApply,onBusyChange}:Props){
 const [name,setName]=useState('我的聊天装扮');const [saved,setSaved]=useState<DecorationPreset[]>([]);const [pending,setPending]=useState<DecorationImport|null>(null);const [parts,setParts]=useState<DecorationPart[]>([]);const [imageUse,setImageUse]=useState<ImageUse>('background');const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [notice,setNotice]=useState('');const fileRef=useRef<HTMLInputElement>(null);const saveReady=useRef(false);
 ...
 const apply=()=>run(async()=>{
  ...
  else if(imageUse==='user'||imageUse==='ai'){const bubble=(await exportCurrent(name)).parts.bubbles!;bubble.name=pending.name;bubble[imageUse].backgroundImage=pending.image;bubble[imageUse].backgroundImageOpacity=1;preset={format:'sullyos-chat-decoration',version:1,name:pending.name,parts:{bubbles:bubble}};selected=['bubbles'];}
  else if(imageUse==='frameUser'||imageUse==='frameAi'){const bubble=(await exportCurrent(name)).parts.bubbles!;bubble.name=pending.name;const side=imageUse==='frameUser'?'user':'ai';bubble[side].frameImage=pending.image;bubble[side].frameSliceTop??=24;bubble[side].frameSliceRight??=28;bubble[side].frameSliceBottom??=24;bubble[side].frameSliceLeft??=28;bubble[side].framePaddingTop??=12;bubble[side].framePaddingRight??=18;bubble[side].framePaddingBottom??=12;bubble[side].framePaddingLeft??=18;bubble[side].frameScale??=.75;bubble[side].frameOpacity??=1;bubble[side].frameRepeat??='stretch';bubble[side].frameMinHeight??=36;preset={format:'sullyos-chat-decoration',version:1,name:pending.name,parts:{bubbles:bubble}};selected=['bubbles'];}
  else if(['backIcon','triggerIcon','triggerActiveIcon','actionsIcon','voiceIcon','voiceActiveIcon','emojiIcon','sendIcon','sendActiveIcon'].includes(imageUse)){
    const map:Record<string,{key:ChatControlKey;field:'icon'|'activeIcon'}>={
      backIcon:{key:'backButton',field:'icon'},triggerIcon:{key:'triggerButton',field:'icon'},triggerActiveIcon:{key:'triggerButton',field:'activeIcon'},
      actionsIcon:{key:'actionsButton',field:'icon'},voiceIcon:{key:'voiceButton',field:'icon'},voiceActiveIcon:{key:'voiceButton',field:'activeIcon'},
      emojiIcon:{key:'emojiButton',field:'icon'},sendIcon:{key:'sendButton',field:'icon'},sendActiveIcon:{key:'sendButton',field:'activeIcon'},
    };
    const target=map[imageUse];const current=(await exportCurrent(name)).parts.controls||{enabled:true};const controls:ChatControlLayout={...current,enabled:true,[target.key]:{...(current[target.key]||{}),[target.field]:pending.image}};
    preset={format:'sullyos-chat-decoration',version:1,name:pending.name,parts:{controls}};selected=['controls'];
  }
  else{const assets:ChatAssetDecorations={[imageUse]:{image:pending.image,opacity:1,x:50,y:50,scale:1,fit:imageUse==='statusBarBackground'||imageUse==='headerBackground'||imageUse==='inputBackground'?'cover':'contain'}};preset={format:'sullyos-chat-decoration',version:1,name:pending.name,parts:{assets}};selected=['assets'];}
 ...
 return <div className="chat-decoration-presets">
  <h3>预设</h3><p className="chat-decoration-note">把布局、自由控件、气泡、背景、图片素材、声音和进阶样式存成一套，随时换上或导出分享。CSS、TXT 和图片也能从这里导入，再选择用途。</p><p className="chat-decoration-note">确认后才会应用到 <b>{target}</b>，没有勾选的部分保持原样。</p>
  ...
  {pending&&<section className="chat-decoration-import-review">
   ...
   {pending.kind==='image'?<><img src={pending.image} className="chat-decoration-background" alt="待导入图片"/><div className="chat-decoration-image-choices">{([['background','聊天背景'],['user','我的气泡贴图'],['ai','角色气泡贴图'],['frameUser','我的图片气泡'],['frameAi','角色图片气泡'],['statusBarBackground','状态栏背景'],['headerBackground','顶栏背景'],['headerDecoration','顶栏装饰'],['inputBackground','输入栏背景'],['inputDecoration','输入栏装饰'],['sendButton','发送按钮背景'],['backIcon','返回图标'],['triggerIcon','小闪电图标'],['triggerActiveIcon','闪电工作中图标'],['actionsIcon','加号图标'],['voiceIcon','语音图标'],['voiceActiveIcon','录音中图标'],['emojiIcon','表情图标'],['sendIcon','发送图标'],['sendActiveIcon','发送生成态图标']] as const).map(([id,label])=><button disabled={busy} key={id} aria-pressed={imageUse===id} onClick={()=>setImageUse(id)}>{label}</button>)}</div></>:<>
    ...
    <details className="chat-decoration-sources"><summary>查看导入内容</summary>{pending.preset.parts.background?.image&&<img className="chat-decoration-background" alt="预设背景" src={pending.preset.parts.background.image}/>}<p>布局：{pending.preset.parts.layout?Object.keys(pending.preset.parts.layout).length+' 项':'未包含'}<br/>自由控件：{pending.preset.parts.controls?Object.keys(pending.preset.parts.controls).length+' 项':'未包含'}<br/>气泡：{pending.preset.parts.bubbles?.name||'未包含'}<br/>图片素材：{pending.preset.parts.assets?Object.keys(pending.preset.parts.assets).length+' 项':'未包含'}<br/>提示音：{pending.preset.parts.sound?.src?.startsWith('data:')?'内嵌音频':pending.preset.parts.sound?.src||'未包含或静音'}</p>{pending.preset.parts.css!==undefined&&<pre className="chat-decoration-code-preview">{pending.preset.parts.css||'（空）'}</pre>}</details>
   </>}
