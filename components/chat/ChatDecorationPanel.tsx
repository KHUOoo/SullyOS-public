...
import ChatAssetEditor from './ChatAssetEditor';
import ChatControlLayoutEditor from './ChatControlLayoutEditor';
...
export type DecorationTab = 'layout'|'bubbles'|'background'|'assets'|'sound'|'css'|'presets';
const tabs: {id:DecorationTab;name:string}[] = [{id:'layout',name:'布局'},{id:'bubbles',name:'气泡'},{id:'background',name:'背景'},{id:'assets',name:'素材'},{id:'sound',name:'声音'},{id:'css',name:'进阶'},{id:'presets',name:'预设'}];
interface Props {
   ...
   {!collapsed&&<>
    ...
    <div className="chat-decoration-body" key={tab}>
     ...
     {tab==='layout'&&<>
      ...
      <h3>顶栏 / 输入栏自由布局</h3>
      <ChatControlLayoutEditor
       key={`controls-${scope}`}
       value={global?theme.chatControlLayout:char.chatControlLayout}
       fallback={global?undefined:theme.chatControlLayout}
       inherited={!global}
       onChange={next=>global?updateTheme({chatControlLayout:next}):updateCharacter({chatControlLayout:next})}
      />
     ...
     {tab==='bubbles'&&<>
      ...
      <button className="chat-decoration-link" onClick={onOpenWorkshop}>打开气泡工坊 · 图片气泡 / 贴纸 / 头像框 →</button>
     </>}
     {tab==='background'&&<>
      <h3>聊天背景</h3>
     ...
     {tab==='assets'&&<>
      ...
      <ChatAssetEditor
       key={scope}
       value={global?theme.chatAssetDecorations:char.chatAssetDecorations}
       fallback={global?undefined:theme.chatAssetDecorations}
       inherited={!global}
       onChange={next=>global?updateTheme({chatAssetDecorations:next}):updateCharacter({chatAssetDecorations:next})}
      />
      <p className="chat-decoration-note">气泡底纹、气泡贴纸和头像框继续由「气泡工坊」管理，它们也会一起进入整套装扮预设。</p>
      <button className="chat-decoration-link" onClick={onOpenWorkshop}>打开气泡工坊 · 图片气泡 / 贴纸 / 头像框 →</button>
     </>}
     {tab==='sound'&&<>
      <h3>消息提示音</h3>
