...
const ChatInputArea: React.FC<ChatInputAreaProps> = ({
    ...
    const actionTiles = [
        <button key="character-photo" onClick={() => onPanelAction('character-photo')} className={`flex flex-col items-center gap-2 active:scale-95 transition-transform ${acnh ? 'text-[#725d42]' : isDiscordStyle ? 'text-slate-200' : 'text-slate-600'}`}>
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm border ${acnh ? 'bg-white/70 border-[#e6dab4] text-[#db72a7]' : isDiscordStyle ? 'bg-slate-800 text-fuchsia-300 border-fuchsia-400/20' : 'bg-fuchsia-50 text-fuchsia-500 border-fuchsia-100'}`}>
                <Camera className="w-6 h-6" weight="fill" />
            </div>
            <span className="text-xs font-bold">拍给我看</span>
        </button>,
            ...
            {selectionMode ? (
                ...
                <div className="sully-chat-composer p-3 px-4 flex gap-3 items-end relative">
                    ...
                    <div className={`sully-chat-input-wrap flex-1 min-w-0 flex items-center px-1 transition-all ${useIOSStandaloneInputFix ? 'overflow-visible' : 'overflow-hidden'} ${inputWrapClass} ${isPixelStyle ? 'focus-within:bg-[#fff7ed]' : isDiscordStyle ? 'focus-within:bg-slate-800 focus-within:border-white/20' : 'border border-transparent focus-within:bg-white focus-within:border-primary/30'}`}>
                        {onVoiceRecorded && (
                            <button type="button" onClick={toggleVoiceRecording} aria-label={isRecordingVoice ? '结束录音并发送' : '发送语音'} title={isRecordingVoice ? '结束录音并发送' : '发送语音'} data-control-state={isRecordingVoice ? 'active' : 'default'} className={`sully-chat-voice-button ml-1 grid h-9 w-9 shrink-0 place-items-center rounded-full transition ${isRecordingVoice ? 'bg-red-500 text-white animate-pulse' : isDiscordStyle ? 'text-slate-400 hover:text-sky-300' : 'text-slate-400 hover:text-primary'} ${voiceSending ? 'opacity-50' : ''}`}>
                    ...
                    <button
                        ...
                        data-control-state={isGenerateButton ? 'active' : 'default'} className={`sully-chat-send-button ${sendButtonClass} ${primaryButtonDisabled ? 'opacity-45 shadow-none' : ''}`}
                    >
                        {sendButtonStyle === 'pill'
