...
import { bubbleFrameContainerStyle, bubbleFrameLayerStyle, hasBubbleFrame } from '../../utils/bubbleNineSlice';
...
const MessageItem = React.memo(({
    ...
    const bubbleFrameUrl = useBlobRefUrl(styleConfig.frameImage);
    ...
    const bubbleFrameActive = hasBubbleFrame(styleConfig);
    const hideBubbleTail = bubbleFrameActive || shouldHideBubbleTail(styleConfig.tailMode, isLastInGroup);
    ...
    const containerStyle: React.CSSProperties = {
        ...
        ...(bubbleFrameActive ? bubbleFrameContainerStyle(styleConfig) : {}),
    };
    ...
    const activeTranslatedContent = translatedContent || '';
    const displayContent = showExpandedTranslation
        ? langAContent
        : (isShowingTarget && (langBContent || activeTranslatedContent)) ? (langBContent || activeTranslatedContent) : langAContent;
    const showTranslateButton = translationEnabled && !showExpandedTranslation && !isUser && (hasBilingual || !!onTranslateToggle);
    ...
    return commonLayout(
        <div className={isVoiceOnlyMsg
            ...
            : `relative ${bubbleFrameActive ? '' : (bubbleVariant === 'flat' || bubbleVariant === 'outline' || bubbleVariant === 'wechat' ? '' : 'shadow-sm ')}${bubbleFrameActive ? '' : 'px-5 py-3 '}${suppressEntranceAnimation ? '' : 'animate-fade-in'} ${bubbleFrameActive || bubbleVariant === 'outline' ? '' : 'border border-black/5 '}active:scale-[0.98] transition-transform overflow-visible ${isUser ? 'sully-bubble-user' : 'sully-bubble-ai'} ${bubbleGroupClasses}`}
            style={isVoiceOnlyMsg ? undefined : containerStyle}>

            {/* Layer 0: Nine-slice bubble skin. This defines the actual bubble chrome when enabled. */}
            {bubbleFrameActive && bubbleFrameUrl && (
                <div
                    className="sully-bubble-nine-slice"
                    style={bubbleFrameLayerStyle(styleConfig, bubbleFrameUrl)}
                />
            )}
